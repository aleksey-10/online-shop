        ...
        switch (action.type) {
            case 'ADD-TO-CART':
                this._state.basket.items.push({
                    item: action.item,
                    // this._state.products.filter(item => item.id == action.id)[0],
                    qtyItem: action.itemQty
                });
                this._state.basket.qty += action.itemQty;
                console.log(action.item, action.itemQty)
                break;
        }
...
export const addToCartActionCreator = (item, itemQty) => (
    {
        type: 'ADD-TO-CART',
        item,
        itemQty
